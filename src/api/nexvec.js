...
export async function ingest({ file, embedding_model }) {
  ...
  if (embedding_model?.trim())
    form.append("embedding_model", embedding_model.trim());
...
export async function getCandidateDetail(resumeId) {
  const res = await fetch(
    `${BASE_URL}/candidates/${encodeURIComponent(resumeId)}`,
  );
  return handleResponse(res);
}
